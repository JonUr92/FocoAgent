 ...
 */
package cargaasignacionautomatica.Class.Mails;
...
public class Mail {
    ...
    public void Insert(ArrayList Personas,String Cedente,String Mandante) throws SQLException{
        ...
        if(ArrayValues.size()>0){
            ...
            String SqlCamposCargas = "merge into campos_cargas_asignaciones as Target\n" +
                        "using (values (GETDATE(),'Mail','"+functions.implode(CamposCarga)+"','"+Cedente+"')) as Source (fecha,tabla,campos,Id_Cedente)\n" +
    ...
    public void PreCarga(ArrayList ArrayValues,ArrayList Campos,ArrayList ArrayValuesCedentes,ArrayList CamposCedentes, String Cedente,String Mandante) throws SQLException{
        ...
        String sql = "";
        DB db = new DB();
            sql = "DELETE FROM Mail_cedente WHERE Id_Cedente = "+Cedente;
            ...
            */
            for (Object Values : ArrayValuesCedentes) {
                sql = "insert into Mail_cedente ("+functions.implode(CamposCedentes)+")  values "+Values.toString();
                ToReturn = db.query(sql, cargaasignacionautomatica.CargaAsignacionAutomatica.Conn);
            }

            /*
                *   Insert Values into Mail Table
            */
            for (Object Values : ArrayValues) {
                sql = "insert into Mail ("+functions.implode(Campos)+")  values "+Values.toString();
                ToReturn = db.query(sql, cargaasignacionautomatica.CargaAsignacionAutomatica.Conn);
            }
    }
    public static ArrayList<String> getColumnas(){
        ...
        DB db = new DB();
        String sql = "select COLUMN_NAME as Field from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'Mail'";
        try {
